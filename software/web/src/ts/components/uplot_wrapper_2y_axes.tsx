...
export class UplotWrapper extends Component<UplotWrapperProps, {}> {
    ...
    componentDidMount() {
        ...
        let options = {
            ...
            axes: [
                ...
                {
                    label: this.props.y_right_label,
                    labelSize: 20,
                    labelGap: 2,
                    labelFont: 'bold 14px system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji"',
                    side: 1,
                    ...
                    filter: (self: uPlot, splits: number[]) => splits.map(v => v % 1 == 0 && v <= this.props.y_right_max ? v : null),
                    ticks: {
                        filter: (self: uPlot, ticks: number[]) => ticks.map(v => v % 1 == 0 && v <= this.props.y_right_max ? v : null),
                    },
                    grid: {show: false},      
                },
            ],
            scales: {
                ...
                right: {
                    from: "left",
                    range: (self: uPlot, fromMin: number, fromMax: number): uPlot.Range.MinMax => [
                        fromMin * this.props.y_right_factor,
                        fromMax * this.props.y_right_factor,
                    ],
                },
            },
            legend: {
    ...
    get_series_opts(i: number, fill: boolean): uPlot.Series {
        ...
        return {
            ...
            spanGaps: false,
            label: name,
            value: (self: uPlot, rawValue: number) => util.hasValue(rawValue) ? util.toLocaleFixed(rawValue, this.use_right_axis(i) ? this.props.y_right_digits : this.props.y_left_digits) + " " + (this.use_right_axis(i) ? this.props.y_right_unit : this.props.y_left_unit) : null,
            stroke: color.stroke,
            ...
            scale: this.use_right_axis(i) ? "right" : "left",
        };
    }
